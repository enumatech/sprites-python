...
contract PaymentChannel {
    ...
    // Utility functions
    modifier onlyplayers {require(playermap[msg.sender] > 0);
        _;}
    ...
    function PaymentChannel(address[2] _players) public {
        for (uint i = 0; i < 2; i++) {
    ...
    function update(uint[3] sig, int r, int[2] _credits, uint[2] _withdrawals) public onlyplayers {
        ...
        bytes32 _h = keccak256(r, _credits, _withdrawals);
        uint8 V = uint8(sig[0]);
